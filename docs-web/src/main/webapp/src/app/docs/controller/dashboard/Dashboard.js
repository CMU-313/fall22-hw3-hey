angular.module('docs').controller('Dashboard', function () {
    ...
    Restangular.one('applicants/list').get({
        ...
        dashboard.applicants = data;

        groupMembersGPA = dashboard.applicants.map(myFunction)

        function myFunction(applicant) {
            return applicant.gpa;
        }

        dashboard.averageGPA = groupMembersGPA.reduce(add, 0);
